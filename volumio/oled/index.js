...
var TIME_BEFORE_DIM = 20000; //  in ms
...
ap_oled.prototype.listen_to = function(api,frequency){
	...
	if(api === "volumio"){
		...
		socket.on("pushState", (data)=> { // se déclenche si changement de morceau / volume / repeat / random / play / pause , ou si l'utilisateur avance manuellement dans la timebar.
			...
			console.log("exit sleep ? ", exit_sleep )
			...
			if( !exit_sleep ){ // Est-ce que l'afficheur devrait passer en mode veille ? 
				...
				clearTimeout(this.DIM_timeout);
				this.DIM_timeout = setTimeout(()=>{
					this.driver.setContrast(0);
					console.log("DIM")
				},TIME_BEFORE_DIM);	
				
			
				this.driver.turnOnDisplay();
				this.driver.setContrast(CONTRAST);
...
fs.readFile("config.json",(err,data)=>{
	...
	if(err) console.log("Cannot read config file. Using default settings instead.");
		...
		try { 
			...
			TIME_BEFORE_DIM = (data && data.dim_after) ? data.dim_after  * 1000 : TIME_BEFORE_DIM
	...
	OLED.driver.begin(
		()=>{
			OLED.driver.buffer.fill(0x00);
			OLED.driver.update(true);
	...
	function start_app(){
		...
		setTimeout( ()=>{
			OLED.playback_mode();
			var volumio_poller = OLED.listen_to("volumio",1000);
